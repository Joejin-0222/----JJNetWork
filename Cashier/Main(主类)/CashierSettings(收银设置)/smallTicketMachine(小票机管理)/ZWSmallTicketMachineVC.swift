...
class ZWSmallTicketMachineVC: UIViewController {

    var DataArray : NSArray = [["当前连接打印机","局域网连接打印机"],["打印尺寸"]]
...
extension ZWSmallTicketMachineVC: UITableViewDelegate{
    
}
extension ZWSmallTicketMachineVC : UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            HeaderView.titleLabel.text = "小票机连接"
        }else{
            HeaderView.titleLabel.text = "尺寸设置"
        }
        return HeaderView
    }
